...
pub fn create_pipeline(
    ...
    let full_shader_source = r#"
        ...
        // NEW: The SSBO binding for our instruction stream
        ...
        Hit map(vec3 p) {
            ...
            // Safety break to prevent infinite GPU hangs
            while (pc < 1000) {
                ...
                if (op == 0.0) { // OP_HALT
                    ...
                    // Unknown instruction, force exit
                    break; 
                }
            }
        ...
        void main() {
            ...
            // Primary Raymarch (with Anti-SDF-Surfing fix!)
            for(int i = 0; i < 256; i++) {
                Hit res = map(ro + rd * t);
                if(res.d < 0.001) {
                ...
                if(t > 100.0) break;
                t += max(res.d, 0.005); // The minimum step-size fix
            }
            ...
            if(hit) {
                ...
                if (material_id == 0) {
                    ...
                    for(int i = 0; i < 100; i++) {
                        ...
                        if(ref_t > 50.0) break;
                        ref_t += max(ref_res.d, 0.005); // Anti-SDF-Surfing fix
                    }
    ...
    let fs_module_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Glsl {
                shader: std::borrow::Cow::Owned(full_shader_source.to_string()),
